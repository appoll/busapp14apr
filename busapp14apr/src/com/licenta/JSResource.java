...
public class JSResource {
	...
	public CustomList getAgencies(@QueryParam ("callback") String callback) {
		...
		for (Entity entity : pq.asIterable()) {
			...
			Agency agency = new Agency(entity.getProperty("agency_id")
					.toString(), entity.getProperty("agency_name").toString(),
					entity.getProperty("agency_url").toString(), entity
							.getProperty("agency_timezone").toString(), entity
							.getProperty("agency_lang").toString(), entity
							.getProperty("agency_phone").toString(), entity
							.getProperty("agency_fare_url").toString());
	...
	public AgencyConverter getAgencyById(@PathParam("id") String id) {
		...
		Agency agency = new Agency(entity.getProperty("agency_id").toString(),
				entity.getProperty("agency_name").toString(), entity
						.getProperty("agency_url").toString(), entity
						.getProperty("agency_timezone").toString(), entity
						.getProperty("agency_lang").toString(), entity
						.getProperty("agency_phone").toString(), entity
						.getProperty("agency_fare_url").toString());
	...
	public CustomList getAgencyRoutes(@PathParam("id") String id) {
		...
		for (Entity entity : pq.asIterable()) {
			...
			if (entity.getProperty("agency_id").toString().equals(id)) {
				Route route = new Route(entity.getProperty("route_id")
						.toString(),
						entity.getProperty("agency_id").toString(), entity
								.getProperty("route_short_name").toString(),
						entity.getProperty("route_long_name").toString(),
						entity.getProperty("route_desc").toString(), entity
								.getProperty("route_type").toString(), entity
								.getProperty("route_color").toString(), entity
								.getProperty("route_text_color").toString());
	...
	@GET
	@Path("agencies/{agency_id}/routes/{id}.json")
	// agency_id nu e relevant
	@Produces({ MediaType.APPLICATION_JSON })
	public RouteConverter getAgencyRouteById(@PathParam("id") String id) {
		...
		Route route = new Route(entity.getProperty("route_id").toString(),
				entity.getProperty("agency_id").toString(), entity.getProperty(
						"route_short_name").toString(), entity.getProperty(
						"route_long_name").toString(), entity.getProperty(
						"route_desc").toString(), entity.getProperty(
						"route_type").toString(), entity.getProperty(
						"route_color").toString(), entity.getProperty(
						"route_text_color").toString());
		RouteConverter converter = new RouteConverter(route);
		return converter;
	}
	...
	@GET
	@Path("agencies/{agency_id}/routes/{route_id}/stops.json")
	// agency_id nu e relevant
	@Produces({ MediaType.APPLICATION_JSON })
	public CustomList getAgencyRouteStops(@PathParam("route_id") String route_id)
		...
		for (Entity trip : tripsPQ.asIterable()) {
			...
			if (trip.getProperty("route_id").toString().equals(route_id)) {
				for (Entity stoptime : stoptimesPQ.asIterable()) {
					if (stoptime.getProperty("trip_id").toString()
							.equals(trip.getProperty("trip_id").toString())) {
						for (Entity stop : stopsPQ.asIterable()) {
							if (stop.getProperty("stop_id")
									.toString()
									.equals(stoptime.getProperty("stop_id")
											.toString())) {
								Stop s = new Stop(
										stop.getProperty("stop_id").toString(),
										stop.getProperty("stop_name")
												.toString(),
										stop.getProperty("stop_desc")
												.toString(),
										stop.getProperty("stop_lat").toString(),
										stop.getProperty("stop_lon").toString(),
										stop.getProperty("zone_id").toString(),
										stop.getProperty("location_type")
												.toString(), stop.getProperty(
												"parent_station").toString());
	...
	@GET
	@Path("agencies/{agency_id}/routes/{route_id}/trips.json")
	// agency_id nu e relevant
	@Produces({ MediaType.APPLICATION_JSON })
	public CustomList getAgencyRouteTrips(@PathParam("route_id") String route_id)
			throws EntityNotFoundException {
		...
		for (Entity trip : tripsPQ.asIterable()) {
			...
			{
			if (trip.getProperty("route_id").toString().equals(route_id)) {
				Trip t = new Trip(trip.getProperty("route_id").toString(), trip
						.getProperty("service_id").toString(), trip
						.getProperty("trip_id").toString(), trip.getProperty(
						"trip_headsign").toString(), trip.getProperty(
						"direction_id").toString(), trip.getProperty(
						"wheelchair_accessible").toString(), trip.getProperty(
						"shape_id").toString());
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public TripConverter getAgencyRouteTripById(
			@PathParam("route_id") String route_id,
			@PathParam("trip_id") String trip_id) {
		DatastoreService datastore = DatastoreServiceFactory
				.getDatastoreService();
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public CustomList getAgencyRouteTripStops(
			@PathParam("route_id") String route_id,
			@PathParam("trip_id") String trip_id) {
		List retstops = new ArrayList();
		...
		for (Entity trip : tripsPQ.asIterable()) {
			...
			if (trip.getProperty("route_id").toString().equals(route_id)
					&& trip.getProperty("trip_id").toString().equals(trip_id)) {
				for (Entity stoptime : stoptimesPQ.asIterable()) {
					if (stoptime.getProperty("trip_id").equals(
							trip.getProperty("trip_id"))) {
						int stop_seq = Integer.parseInt(stoptime.getProperty(
								"stop_sequence").toString());
						for (Entity stop : stopsPQ.asIterable())
							if (stop.getProperty("stop_id").equals(
									stoptime.getProperty("stop_id"))) {
								Stop s = new Stop(
										stop.getProperty("stop_id").toString(),
										stop.getProperty("stop_name")
												.toString(),
	...
	@GET
	@Path("agencies/{agency_id}/stops/{id}.json")
	// agency_id nu e relevant
	@Produces({ MediaType.APPLICATION_JSON })
	public StopConverter getAgencyStopById(@PathParam("id") String id) {
		...
		Stop stop = new Stop(entity.getProperty("stop_id").toString(), entity
				.getProperty("stop_name").toString(), entity.getProperty(
				"stop_desc").toString(), entity.getProperty("stop_lat")
				.toString(), entity.getProperty("stop_lon").toString(), entity
				.getProperty("zone_id").toString(), entity.getProperty(
				"location_type").toString(), entity.getProperty(
				"parent_station").toString());
	...
	// duplicate
	@GET
	@Path("agencies/{agency_id}/stops/{stop_id}/routes.json")
	// agency_id nu e relevant
	@Produces({ MediaType.APPLICATION_JSON })
				...
				for (Entity trip : tripsPQ.asIterable())
					...
					if (stoptime.getProperty("trip_id").equals(
						...
						for (Entity r : routesPQ.asIterable())
							if (r.getProperty("route_id").equals(
									trip.getProperty("route_id"))) {
								Route route = new Route(
										r.getProperty("route_id").toString(),
										r.getProperty("agency_id").toString(),
										r.getProperty("route_short_name")
												.toString(),
										r.getProperty("route_long_name")
												.toString(),
										r.getProperty("route_desc").toString(),
										r.getProperty("route_type").toString(),
										r.getProperty("route_color").toString(),
										r.getProperty("route_text_color")
												.toString());
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public CustomList getAgencyStopPredictions(
			@PathParam("stop_id") String stop_id) {
		...
		for (Entity stoptime : stoptimesPQ.asIterable())
			if (stoptime.getProperty("stop_id").toString().equals(stop_id))
				for (Entity trip : tripsPQ.asIterable())
					if (Utils.checkService (trip.getProperty("service_id").toString()))
					{
					if (trip.getProperty("trip_id").equals(
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public CustomList getAgencyStopRoutePredictions(
			@PathParam("stop_id") String stop_id,
			@PathParam("route_id") String route_id) {
		List retpredictions = new ArrayList();
		DatastoreService datastore = DatastoreServiceFactory
