			...
			if (stoptime.getProperty("stop_id").toString().equals(stop_id))
				for (Entity trip : tripsPQ.asIterable())
					if (trip.getProperty("trip_id").equals(stoptime.getProperty("trip_id")))
					{
						...
						Prediction next_hour = new Prediction (trip.getProperty("trip_id").toString(),
																Utils.secondsToArrivalNextHour(stoptime.getProperty("arrival_time").toString()),
																trip.getProperty("route_id").toString());
						if (p.getSeconds()>0) retpredictions.add(p);
						if (next_hour.getSeconds()>0) retpredictions.add(next_hour);
					}
